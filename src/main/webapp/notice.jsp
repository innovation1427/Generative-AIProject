...
<section id="cat2">
   ...
   <c:forEach items="${nList}" var="notice">
      <tr>
         <td>${notice.num}</td>
         <td><a href="notice2.do?num=${notice.num}">${notice.title}</a></td>
...
<div class="pagination">
    <c:if test="${currentPage == 1}">
        <a href="notice.do?page=${currentPage}">&laquo; 이전</a>
    </c:if>
    <c:if test="${currentPage > 1}">
        <a href="notice.do?page=${currentPage - 1}">&laquo; 이전</a>
    </c:if>
        ...
        <c:when test="${totalPages <= 10}">
            ...
            <c:forEach begin="1" end="${totalPages}" varStatus="loop">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="notice.do?page=${loop.index}">${loop.index}</a>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
        </c:when>
    ...
    <c:if test="${currentPage < totalPages}">
        <a href="notice.do?page=${currentPage + 1}">다음 &raquo;</a>
    </c:if>
    <c:if test="${currentPage == totalPages}">
        <a href="notice.do?page=${currentPage}">다음 &raquo;</a>
    </c:if>
    <c:if test="${sessionScope.role eq '관리자'}">
        		<a href="notice3.jsp">글쓰기</a>
    			</c:if>
</div>
